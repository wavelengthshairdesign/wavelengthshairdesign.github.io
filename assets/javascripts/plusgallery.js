                ...
                $.getJSON(albumURL, function (json) {
                    ...
                    switch (pg.type) {
                        ...
                        case 'google':
                            ...
                            //include only specified albums if pg.include is set and sort them in order
                            sortedObjPath = [];
                            if (albumTotal > 0) {
                                if (pg.include !== null) {
                                    $.each(pg.include, function (index, value) { //add 'em in sorted order
                                        $.each(objPath, function (i, obj) {
                                            //obj is entry
                                            if (i < albumTotal) {
                                                galleryJSON = obj.link[0].href;
                                                if (galleryJSON.indexOf(value) > 0) {
                                                    sortedObjPath.push(obj);
                                                }
                                            }
                                        });
                                    });
                                }
                                objPath = sortedObjPath;
                            }
                            ...
                            if (i < pg.limit) {
                                switch (pg.type) {
                                    case 'google':
                                        ...
                                        var blockImageFileAsTitle = /\.(jpg|png|gif)$/ig.test(imgTitle);

                                        if (blockImageFileAsTitle) {
                                            imgTitle = "";
                                        }
